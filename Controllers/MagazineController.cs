...
namespace TCSA.OOP.LibraryManagementSystem.Controllers
{
    internal class MagazineController : IBaseController
    {
        public void ViewItems()
        {
            ...
            var magazines = MockupDatabase.LibraryItems.OfType<Magazine>();
        ...
        public void AddItem()
        {
            ...
            if (MockupDatabase.LibraryItems.OfType<Magazine>().Any(m => m.Name.Equals(title, StringComparison.OrdinalIgnoreCase)))
            {
                AnsiConsole.MarkupLine("[red]This magazine already exists.[/]");
            }
            else
            {
                var newMagazine = new Magazine(MockupDatabase.LibraryItems.Count + 1, title, publisher, publishDate, location, issueNumber);
                MockupDatabase.LibraryItems.Add(newMagazine);
        ...
        {
            if (MockupDatabase.LibraryItems.OfType<Magazine>().Count() == 0)
            {
            ...
            var magazineToDelete = AnsiConsole.Prompt(
                new SelectionPrompt<Magazine>()
                    ...
                    .AddChoices(MockupDatabase.LibraryItems.OfType<Magazine>()));

            if (MockupDatabase.LibraryItems.Remove(magazineToDelete))
            {
